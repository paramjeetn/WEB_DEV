...
app.get("/filter",(req,res)=>{ 
  const keyValue =req.query.type;
  const foundJoke=jokes.filter((joke)=>joke.jokeType===keyValue);
  res.json(foundJoke);
});
...
app.post("/jokes",(req,res)=>{
  const body=req.body;
  const joke=body.jokeText;
  const jokeType=body.jokeType;
...
app.patch("/jokes/:id",(req,res)=>{
  ...
  const replacementJoke = {
    id: id,
    jokeText: req.body.jokeText || existingJokes.jokeText,
    jokeType: req.body.jokeType || existingJokes.jokeType,
  };
  const searchIndex = jokes.findIndex((joke) => joke.id === id);
  jokes[searchIndex] = replacementJoke;
  res.json(replacementJoke);
...
app.delete("/jokes/all",(req,res)=>{
  ...
  if(key===masterKey){
    ...
    res
    .status(404)
    .send("Sorry, API Key is unauthorised")
  }
